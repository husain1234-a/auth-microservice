-- Updated Cart Microservice Database Schema
-- This schema is designed for isolated cart service database with no foreign key constraints

-- Users table (denormalized - user info stored locally but synced from auth service)
CREATE TABLE users (
...
-- Categories table (denormalized - category info stored locally but synced from product service)
...
-- Products table (denormalized - product info stored locally but synced from product service)
...
CREATE TABLE cart_items (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    PRIMARY KEY (id),
    -- Removed foreign key constraints to carts and products tables
...
CREATE TABLE wishlist_items (
    ...
    updated_at timestamp with time zone DEFAULT now(),
    PRIMARY KEY (id),
    -- Removed foreign key constraints to wishlists and products tables
...
CREATE TABLE cart_promo_codes (
    ...
    PRIMARY KEY (id),
    -- Removed foreign key constraints to carts and promo_codes tables
