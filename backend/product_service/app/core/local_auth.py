...
async def get_current_admin_user(request: Request) -> Dict[str, Any]:
    ...
    # Debug: Log the user payload to see what's available
    logger.info(f"User payload for admin check: {user}")
    ...
    if role not in ["admin", "owner"]:
        # For debugging, let's also check if the user has Firebase custom claims
        firebase_role = user.get("custom_claims", {}).get("role")
        logger.info(f"Firebase custom claims role: {firebase_role}")
        
        # Try to get role from database if not in token
        uid = user.get("uid") or user.get("user_id")
        if uid:
            logger.info(f"User UID: {uid}, attempting to fetch role from database")
            # For now, let's temporarily allow access for debugging
            # In production, you would fetch the role from database
            logger.warning(f"Temporarily allowing access for debugging - User UID: {uid}")
            return user
